...
var PieChart = new Class({
	options: {
		pieChartRadius: 140,  // Half the width of your pie chart
	...
	makePieChart: function(){
    	...
    	var tds, data = [], color, colors = [], label, labels = [], value = 0, values = [], list, lists = [], total = 0;
		...
		for (var i = 0; i < trs.length; i++) {
			...
			list = tds[this.options.td_list_index].innerHTML;
			lists[colors.length] = list; // Save value for tooltips
    	...
    	for (piece=0; piece < tableLength; piece++) {
			...
			new Element('area', {
				...
				'title': labels[piece] + ' :: ' + values[piece] + '<br/>' + lists[piece]
